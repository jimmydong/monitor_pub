...
class DB_Sql {
  ...
  function connect($Database = "", $Host = "", $User = "", $Password = "") {
    ...
    if($need_connect==1){
        ...
        if($this->Lang) mysql_query("SET NAMES '".$this->Lang."'", $this->Link_ID);
  ...
  /* by jimmy, in case timeout 'mysql has gone'. -- I have adjust connect(), so it's JUST SAME AS connect() */
  function reconnect($Database = "", $Host = "", $User = "", $Password = "") {
    /* Handle defaults */
    if ("" == $Database)
      $Database = $this->Database;
    if ("" == $Host)
