...
class SysPager {
	...
	/** 分页显示模板，0：平铺；1：平铺+GO；2：下拉框；3：下拉框+GO； */
	...
	/** 网页的链接地址格式，默认是 POST 方式处理，常用的格式：?page=${PAGE}，可以使用的变量有：${PAGE}和${PAGE_SIZE} */
	public $url_html = '';
	/** 不可用的按钮（span） */
	...
	private function createPagerHtml()
	{
		...
		// 中间区域的分页显示
		if ($this->page_template == 2 || $this->page_template == 3)
		{	// 下拉框的模板效果
	...
	{
		return '<span class="'.$this->css_unabled.'">' . $text . '</span>' . chr(13);
	}
	...
	private function createLinkButton($page)
	{
		...
		return '<a href="' . $url . '" target="' . $this->target . '">' . $text . '</a>' . chr(13);
	}
	...
	private function createCurLinkButton()
	{
		...
		if ($this->page_template == 2 || $this->page_template == 3)
		{
			return '<span class="'.$this->css_cur_select.'">' . $text . '</span>' . chr(13);
		}
		else 
		{
			return '<span class="'.$this->css_cur.'">' . $text . '</span>' . chr(13);
		}
	}
	...
	private function createCurSelect()
	{
		$text = str_replace('${PAGE}', $this->_page_index, $this->cur_bar_html);
		$text = str_replace('${TOTAL_PAGE}', $this->total_page, $text);
		return '<span class="'.$this->css_cur_select.'" onclick="showPagerSelect(event, "' . $this->id . '")">'.$text.'</span>' . chr(13);
	}
	...
	{
		$html = '';
		if (empty($this->url_html) || $this->page_template == 1 || $this->page_template == 2 || $this->page_template == 3)
		{	// POST方式访问、需要显示 GO、使用下拉框显示
			if ($this->page_template == 1 || $this->page_template == 3)
			{
				$html .= '<input type="text" size="2" class="'.$this->css_input_text.'" id="pager_form_page2_'.$this->id.'" name="page" value="'.$this->_page_index.'" /><input type="button" class="'.$this->css_input_button.'" value="GO" onclick="showPagerSelectPage(document.getElementById(\'pager_form_page2_'.$this->id.'\').value, "' . $this->id . '")" />' . chr(13);
			}
			if (!empty($this->url_html))
			{
				$url = str_replace('${PAGE}', $this->_page_index, $this->url_html);
				$url = str_replace('${PAGE_SIZE}', $this->_page_size, $url);
			}
			$html .= '<form action="'.$url.'" id="pager_form_'.$this->id.'" style="display:none;" target="" method="post"><input type="hidden" id="pager_form_page_'.$this->id.'" name="page" /></form>' . chr(13);
		}
		
		if ($this->page_template == 2 || $this->page_template == 3)
		{	// 下拉框的模板效果
			$html .= '<div id="pager_'.$this->id.'" class="'.$this->css_more_select.'" ';
			$html .= 'onmouseover="document.getElementById(\'pager_'.$this->id.'\').style.display=\'block\'" onmouseout="document.getElementById(\'pager_'.$this->id.'\').style.display=\'none\'">';
			for ($i = $start_page; $i <= $end_page; $i++)
			{
				if ($i == $this->_page_index)
				{
					$html .= '<div>'.$this->createCurLinkButton().'</div>'.chr(13);
				}
				else 
				{
					$html .= '<div>'.$this->createLinkButton($i).'</div>'.chr(13);
				}
			}
			$html .= '</div>';
		}
